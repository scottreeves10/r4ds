...
1.  <span style="color:blue">How could you rescale the count dataset above to more clearly show
    the distribution of cut within colour, or colour within cut?</span>

1.  <span style="color:blue">Use `geom_tile()` together with dplyr to explore how average flight
    delays vary by destination and month of year.  What makes the 
    plot difficult to read? How could you improve it?</span>

1.  <span style="color:blue">Why is it slightly better to use `aes(x = color, y = cut)` rather
    than `aes(x = cut, y = color)` in the example above?</span>
...
1.  <span style="color:blue">Instead of summarising the conditional distribution with a boxplot, you
    ...
    the 2d distribution of `carat` and `price`?</span>

1.  <span style="color:blue">Visualise the distribution of carat, partitioned by price.</span>

1.  <span style="color:blue">How does the price distribution of very large diamonds compare to small 
    diamonds. Is it as you expect, or does it surprise you?</span>
    
1.  <span style="color:blue">Combine two of the techniques you've learned to visualise the 
    combined distribution of cut, carat, and price.</span>

1. <span style="color:blue">Two dimensional plots reveal outliers that are not visible in one 
   ...
   even though their `x` and `y` values appear normal when examined separately.</span>

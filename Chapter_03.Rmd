...
## Filter() - Filter rows with `filter()` {#filter}
...
## Arrange() - Arrange rows with `arrange()`
...
## Select() - Select columns with `select()` {#select}
...
## Mutate() - Add new variables with `mutate()` {#mutate}
...
## Summarize() - Grouped summaries with `summarise()`
...
### Pipe %>% Combining multiple operations with the pipe
...
1.  Which plane (`tailnum`) has the worst on-time record?
    ...
    <!-- The first output below shows N988AT at the top of a descending arrangment. The next output shows N988AT as specifically the top 1.  -->
    ...
    The chunk below does not work for the purpose of this excercise. However, it is kept for the purpose of illustrating that max() applies to each summarized group row - i.e., rather than the column vector of the variable of all the summarized rows.  

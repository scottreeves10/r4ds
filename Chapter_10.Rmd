...
1.  <span class="question">We know that some days of the year are "special", and fewer people than usual fly on them. How might you represent that data as a data frame? What would be the primary keys of that table? How would it connect to the existing tables?</span>
    ...
    New table `special_days` with passenger_volume, including and connected by year, month day. More granularly it could include and be conneted by faa in airports, etc. 
...
1.  Add a surrogate key to `flights`.
    ...
    ```{r}
    flights %>% 
      mutate(flights_uid = row_number()) %>%      # surrogate key
      select(flights_uid, everything()) %>%       # show results...
      glimpse()
    ```
